...
app.get('/enrollment/:id/transaction_history', async (req, res) => {
    try {
        ...
        transactionsSnapshot.forEach(doc => {
            ...
            // Fix: Tiyakin na string ang data.description bago mag-check
            const desc = String(data.description || '').toLowerCase();
            const transactionType = desc.includes("final") ? "Final Installment" : "Downpayment/Partial Payment";
            
            // Final Fix: Gumamit ng nullish coalescing para hindi mag-crash sa substring
            const dateString = data.transaction_timestamp ?? 'N/A'; 
            let formattedDate = 'N/A';
            if (dateString !== 'N/A' && typeof dateString === 'string' && dateString.length >= 10) {
                 formattedDate = dateString.substring(0, 10);
            }

            transactions.push({
                "transaction_id": data.transaction_id,
                "date": formattedDate, // Gumamit ng formattedDate
