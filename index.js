...
const getFeesInfo = async (enrollmentId) => {
    ...
    let totalPaid = 0;
    transactionsSnapshot.forEach(doc => {
        totalPaid += doc.data().amount || 0; // Dagdag safety
    });
...
app.get('/transactions/:transaction_id', async (req, res) => {
    try {
        ...
        res.status(200).json({
            "transaction_id": data.transaction_id,
            "date": data.transaction_timestamp || 'N/A', // Safety check
            "student_id": "S-2023-005",
            "amount_paid": data.amount || 0, // Safety check
            "payment_method": data.payment_method || 'N/A', // Safety check
            "reference_number": data.transaction_ref || 'N/A',
            "status": getStatusDescription(data.payment_status_id)
        });
    } catch (error) {
...
app.get('/enrollment/:id/transaction_history', async (req, res) => {
    try {
        ...
        transactionsSnapshot.forEach(doc => {
            ...
            // Safety check: Tiyakin na ang amount ay valid number bago idagdag
            ...
            // Dito ang pinakamalaking fix: Tiyakin na String ang data.description bago gamitin ang includes()
            const desc = String(data.description || '').toLowerCase();
            const transactionType = desc.includes("final") ? "Final Installment" : "Downpayment/Partial Payment";
            
            // Safety check: Tiyakin na may timestamp bago i-substring
        ...
        // Ito ang critical: Ang error ay galing sa transaction loop. I-log ito.
