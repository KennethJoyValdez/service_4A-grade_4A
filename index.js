...
// Helper function (Only defined ONCE)
...
const getFeesInfo = async (enrollmentId) => {
    ...
    return {
        "enrollment_id": numericEnrollmentId, "student_id": "S-2023-005", "term": "Fall 2024", "currency": "PHP",
...
// GET /enrollment/{id}/fees_information (TEST 1)
...
// POST /enrollment/{id}/payment_transactions (TEST 2)
...
// POST /transactions/{transaction_id} (TEST 3)
...
// GET /transactions/{transaction_id} (TEST 4)
...
// GET /transactions (NEW/REPLACED TEST 5: Listahan ng Lahat)
...
// GET /enrollment/{id}/transaction_history (DEPRECATED/BUGGY - Hindi na gagamitin sa demo)
app.get('/enrollment/:id/transaction_history', async (req, res) => {
    // Para lang ito sa pag-maintain ng compatibility sa lumang code, pero hindi gagamitin sa demo
    console.warn(`Attempted access to deprecated endpoint: /enrollment/${req.params.id}/transaction_history`);
    return res.status(500).json({ message: "Endpoint temporarily disabled due to data integrity issues." });
});
